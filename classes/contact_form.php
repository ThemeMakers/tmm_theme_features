...
if (! class_exists('TMM_Contact_Form')) {
	class TMM_Contact_Form {
		...
		public static function contact_form_request() {
			...
			//*****
			if ( empty( $website ) ) {
				$website = esc_html__( "Email from contact form", 'accio' );
			}
			...
			//*****
			$after_message = "\r\n<br />--------------------------------------------------------------------------------------------------\r\n<br /> " . esc_html__( 'This mail was sent via', 'accio' ) . " " . site_url() . " " . esc_html__( 'contact form', 'accio' );
