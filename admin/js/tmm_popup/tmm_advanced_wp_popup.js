var TMM_ADVANCED_WP_POPUP = function () {
  var self = {
    ...
    popup: function (params) {
      ...
      jQuery("#tmm_advanced_wp_popup").append(
        '<li class="tmm_advanced_wp_popup_li" style="width:' +
          params.width +
          "px;height:" +
          params.height +
          "px;margin-left:-" +
      ...
      if (params.title.length > 0) {
        ...
        popup
          .children(".tmm_titlebar")
          .append(
            '<a href="javascript:void(0);" class="advanced_wp_popup_close"></a>'
          );
      }
      /***/
      ...
      jQuery(popup).on(
        "keyup",
        ".advanced_wp_popup_content input[type=text]",
        function () {
      ...
      jQuery(popup).on(
        "click",
        ".advanced_wp_popup_content input[type=checkbox]",
        function () {
      ...
      jQuery(popup).on(
        "keyup",
        ".advanced_wp_popup_content textarea",
        function () {
      ...
      jQuery(popup).on(
        "change",
        ".advanced_wp_popup_content select",
        function () {
      ...
      jQuery.each(params.buttons, function (index, button) {
        if (button.action == "close") {
          ...
          jQuery(popup_li)
            ...
            .append(
              '<a href="javascript:void(0);" class="tmm_button advanced_wp_popup_close">' +
                button.name +
                "</a>"
            );
          return;
        ...
        jQuery(popup_li)
          .find(".advanced_wp_popup_buttons")
          .append(
            '<a href="javascript:tmm_advanced_wp_popup.do_action(' +
              index +
              ');void(0);" class="tmm_button">' +
              button.name +
              "</a>"
          );
      });
